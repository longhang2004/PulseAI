...
export class KafkaProducerService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.kafka = new Kafka({
      clientId: 'diagnosis-service-producer',
      brokers: brokers.split(','),
    });
    this.producer = this.kafka.producer();
    try {
      await this.producer.connect();
      console.log('Diagnosis Kafka Producer connected successfully');
    } catch (err) {
      console.error('Failed to connect Diagnosis Kafka Producer:', err);
    }
  }
